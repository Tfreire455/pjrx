      ...
      <div className="relative z-10 flex flex-col h-full">
         <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
                <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-medium text-emerald-400">
                    Active
                </span>
         ...
         <div className="mt-5">
            ...
            <p className="text-sm text-zinc-500 mt-2 line-clamp-2 leading-relaxed">
              {p.description || "Projeto sem descrição definida."}
            </p>
         </div>
         ...
         <div className="mt-auto pt-6">
            <div className="flex items-center justify-between text-xs text-zinc-600 mb-2 font-mono">
                <span>PROGRESS</span>
                <span className="group-hover:text-violet-400 transition-colors">0%</span>
            </div>
            <div className="h-1 w-full bg-zinc-800/50 rounded-full overflow-hidden">
               <div className="h-full bg-violet-500 w-[5%] group-hover:w-[15%] transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(139,92,246,0.5)]" />
            </div>
         </div>
      </div>
    </motion.div>
...
// --- NOVO COMPONENTE: SELECTOR DE WORKSPACE CUSTOMIZADO ---
  ...
  return (
    <div className="relative group min-w-[200px]" ref={containerRef}>
      ...
      <div 
        ...
        className="flex items-center gap-4 cursor-pointer select-none group/trigger"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-4 w-[320px] p-2 rounded-[1.5rem] border border-white/10 bg-[#0a0a0a]/90 backdrop-blur-2xl shadow-2xl shadow-black/50 z-50 flex flex-col gap-1 max-h-[400px] overflow-hidden"
          >
            <div className="px-4 py-3 text-[10px] font-bold text-zinc-500 uppercase tracking-widest bg-white/[0.02] border-b border-white/5 mb-1">
            ...
            <div className="overflow-y-auto custom-scrollbar p-1 flex flex-col gap-1 max-h-[300px]">
              {workspaces.map(w => (
                <div
                  ...
                  className={`group/item relative flex items-center justify-between px-4 py-3.5 rounded-xl cursor-pointer transition-all duration-200 ${
                    w.id === activeId 
                      ? 'bg-violet-500/10 text-white border border-violet-500/20 shadow-[0_0_15px_-5px_rgba(124,58,237,0.3)]' 
                ...
                >
                  ...
                  {w.id === activeId && (
                    <motion.div layoutId="check" className="text-violet-400">
                      <Check size={16} />
                    </motion.div>
                  )}
                </div>
            ...
            <div className="p-2 border-t border-white/5 mt-auto bg-[#0a0a0a]">
              <button
                ...
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-zinc-400 hover:bg-zinc-900 hover:text-white transition-all text-sm font-medium w-full text-left group/btn"
              >
                <div className="flex items-center justify-center h-6 w-6 rounded-lg bg-white/5 border border-white/10 group-hover/btn:border-violet-500/30 group-hover/btn:bg-violet-500/10 group-hover/btn:text-violet-400 transition-colors">
                  <Plus size={14} />
                </div>
...
export function Dashboard() {
  ...
  // Handle Workspace Change do componente customizado
  ...
  return (
    ...
    <div className="relative min-h-screen pb-20">
      ...
      <motion.div 
        ...
        initial="hidden"
        animate="show"
        className="space-y-10 max-w-[1600px] mx-auto px-6 sm:px-8 md:px-12 py-8"
      >
        {/* === HEADER SECTION === */}
        <motion.div variants={item} className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between border-b border-white/5 pb-8">
          <div className="flex items-center gap-6">
            ...
            <div className="relative group">
              ...
              <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-700 shadow-2xl shadow-violet-500/30 text-white border border-white/10">
                <Building2 size={32} />
              </div>
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2 text-xs font-bold text-violet-400 uppercase tracking-widest">
                <Sparkles size={10} /> Workspace Ativo
              </div>
          ...
          <div className="flex items-center gap-3">
             <Button variant="ghost" size="sm" onClick={handleCreateWorkspace} disabled={isCreatingWs} className="h-10 rounded-full text-zinc-400 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10">
              <Plus className="mr-2 h-4 w-4" /> Novo Workspace
            </Button>
            <Button size="sm" onClick={() => setProjectModalOpen(true)} className="h-10 px-6 bg-white text-black hover:bg-zinc-200 rounded-full font-semibold shadow-[0_0_20px_-5px_rgba(255,255,255,0.3)] transition-all hover:scale-105">
